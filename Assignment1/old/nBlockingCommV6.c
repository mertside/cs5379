...
int main(int argc, char **argv)
{
  ...
  if(pid == 0) {			/*** pid == 0 ***/
    ...
    mtag = 4;
    MPI_Isend(&data[2500], 2500, MPI_INT, 1, mtag, MPI_COMM_WORLD, &req_a);
    ...
    /*** Send first 25 computed row_sums to pid 0 ***/
    mtag = 3;
    MPI_Isend(row_sum, 25, MPI_INT, 0, mtag, MPI_COMM_WORLD, &req_r);
    
    mtag = 4;
    MPI_Recv(&data[2500], 2500, MPI_INT, 0, mtag, MPI_COMM_WORLD, &status);

    // sum data[] from 25 to 49
    for(i = 25; i < 50; i++) {
      row_sum[i] = 0;
      for(j = 0; j < 100; j++)
         row_sum[i] += data[i][j];
    ...
    /*** Send remaining computed row_sums to pid 0 ***/
    mtag = 2;
