// CS5379: Parallel Processing - Assignment 3
// 
...
{
  int pid, np;
  int i, j, count, tmp, least, leastPos, *found, *allLeasts, *allLeastPos;
...
struct LeastGroup getLeastOfAllLeasts(struct LeastGroup group, int pid, int np)
{
  ...
  if (pid == 0)
  {
    ...
    {
      int least, leastPos;
      // Recv least from process i and put in ith position in groups
      MPI_Recv(&least, 1, MPI_INT, i, 0, MPI_COMM_WORLD, &status); 
      // Recv least's position from process i and put in ith position in groups
      MPI_Recv(&leastPos, 1, MPI_INT, i, 1, MPI_COMM_WORLD, &status);
  ...
  else
  {
    ...
    // Send least to process 0
    MPI_Send(&(group.least), 1, MPI_INT, 0, 0, MPI_COMM_WORLD); 
    // Send leastPos to process 0
    MPI_Send(&(group.leastPos), 1, MPI_INT, 0, 1, MPI_COMM_WORLD); 

    // Recv the least group from pid 0 
    int least, leastPos;
    // Recv least from process 0 and put in group
    MPI_Recv(&least, 1, MPI_INT, 0, 0, MPI_COMM_WORLD, &status);
    // Recv least's position from process 0 and put in group
    MPI_Recv(&leastPos, 1, MPI_INT, 0, 1, MPI_COMM_WORLD, &status);
